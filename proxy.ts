...
export async function proxy(request: NextRequest) {
  ...
  if (session && pathname !== '/banned' && !pathname.startsWith('/api/webhooks')) {
    const { data: profile } = await supabase
      .from('users').select('is_banned').eq('id', session.user.id).maybeSingle()
  ...
  if (pathname.startsWith('/admin')) {
    if (!session) return NextResponse.redirect(new URL('/login?redirectTo=/admin', request.url))
    if (role !== 'admin') return NextResponse.redirect(new URL('/', request.url))
  }

  // /dashboard — seller + admin
  if (pathname.startsWith('/dashboard')) {
    if (!session) return NextResponse.redirect(new URL('/login?redirectTo=/dashboard', request.url))
