  ...
  return (
    <div className="min-h-screen bg-[#08080E] text-[#F0EDE6]">
      <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-[#08080E]/85 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          {user ? (
            ...
            <Link href="/shop" className="text-xs text-gray-400 hover:text-[#C9A84C] transition-colors">المتجر العام</Link>
          )}
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-6 pt-28 pb-24">
        {/* STORE HEADER */}
        <div className="bg-[#111118] border border-white/5 rounded-3xl p-8 mb-12 flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-1.5 flex-wrap">
              ...
              {store.stripe_verified && (
                <span className="text-[10px] bg-[#4F8EF7]/10 text-[#4F8EF7] px-2.5 py-1 rounded-full font-bold">✓ موثّق</span>
              )}
            </div>
            {store.bio && <p className="text-gray-500 text-sm leading-relaxed mb-3 max-w-xl">{store.bio}</p>}
            <div className="flex items-center gap-4 text-xs text-gray-500">
              {store.rating_avg && <span>★ {store.rating_avg.toFixed(1)} ({store.rating_count} تقييم)</span>}
              <span>{store.sales_count ?? 0} مبيعة</span>
              <span>عضو منذ {new Date(store.created_at).getFullYear()}</span>
            </div>
          </div>
        </div>

        {/* LISTINGS */}
        <h2 className="text-xl font-serif font-bold mb-6">منتجات المتجر ({listings?.length ?? 0})</h2>
        {listings && listings.length > 0 ? (
          ...
          <div className="text-center py-20 text-gray-600 text-sm">لا توجد منتجات منشورة لهذا المتجر بعد</div>
        )}
      </main>
    </div>
  )
