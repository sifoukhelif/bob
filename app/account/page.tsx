...
function sanitizeUsername(raw: string) {
  // يقبل: أحرف إنجليزية كبيرة/صغيرة، أرقام، شرطة سفلية، ومسافات بين الكلمات
  // يحذف أي رمز غير مسموح به، ويُزيل المسافات الزائدة من الأطراف
  const clean = raw.replace(/[^a-zA-Z0-9_ ]/g, '').trim()
  // يمنع مسافات متعددة متتالية في الوسط (يُبقي مسافة واحدة فقط)
...
async function updateUsername(formData: FormData) {
  ...
  if (clean.length < 3) {
    redirect('/account?error=' + encodeURIComponent('اسم المستخدم يجب أن يكون 3 أحرف على الأقل (إنجليزي/أرقام فقط)'))
  }
        ...
        <form action={updateUsername} className="bg-[#111118] border border-white/6 rounded-2xl p-6 flex flex-col gap-4 max-w-md">
          <div>
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-[#C9A84C]/40 transition-colors"
            />
            <p className="text-[10px] text-gray-600 mt-1.5">أحرف إنجليزية، أرقام، وشرطة سفلية فقط — يظهر هذا الاسم بدل بريدك في الموقع.</p>
          </div>

          {error && <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3 text-xs text-red-400">{error}</div>}
          {success === '1' && <div className="bg-[#2ECC9A]/10 border border-[#2ECC9A]/20 rounded-xl px-4 py-3 text-xs text-[#2ECC9A]">تم حفظ اسم المستخدم بنجاح</div>}
