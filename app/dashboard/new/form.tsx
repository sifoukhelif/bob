  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (type === 'product' && file) {
        ...
        const { error: uploadError } = await sb.storage.from('listing-files').upload(path, file, { upsert: true })
        if (uploadError) {
          throw new Error('تم إنشاء المنتج، لكن تعذّر رفع الملف. أضفه من صفحة المنتج.')
        }
